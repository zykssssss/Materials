...
#     #print(f"ImageName {image_name} 的DEM值是：{dem_value}")
#     return dem_value
...
for epoch in range(num_epochs):
    ...
    for image_names, images, labels in tqdm(train_loader):
        ...
        constraint_variable = get_constraint_variable(predicted, image_names)
        
        loss = torch.add(loss, constraint_variable)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for image_names, images, labels in val_loader:
            ...
            processed_predictions = torch.clone(predicted)

            for i in range(probabilities.size(0)):
                if probabilities[i].max() < 0.7:
                    processed_predictions[i] = get_label(df, image_names[i])

            
            val_total += labels.size(0)
            val_correct += (processed_predictions == labels).sum().item()
            all_predictions.extend(processed_predictions.tolist())
